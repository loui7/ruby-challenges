...
def exes_and_ohs(input)
  input_arr = input.split("").map(&:downcase)
  char_a = input_arr[0]
  char_b = input_arr.bsearch{|char| char != char_a}
  # Guard clause to return false if there are more than 2 characters in the string
  return false unless input_arr.bsearch{|char| char != char_a && char != char_b}.nil?

  return input_arr.count(char_a) == input_arr.count(char_b)
end
